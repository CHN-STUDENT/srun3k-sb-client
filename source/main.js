$(function() {
	...
	function showErrorMessage(error, success, redirect) {
		...
		if (typeof redirect != "undefined" && redirect) {
			layer.alert(message, { icon: icon, skin: 'layui-layer-molv', btn:[(translate['OK']||"OK")], title:translate['Info']||"Ingo" }, function(){
				location.href = "./";
			});
			return;
		}
		layer.alert(message, { icon: icon, skin: 'layui-layer-molv', btn:[(translate['OK']||"OK")], title:translate['Info']||"Info" });
	}
	...
	function showLog(username) {
		$.Log({username:username}, function(data) {
			...
			layer.alert(message, { icon: 2, skin: 'layui-layer-molv', btn:[(translate['OK']||"OK")], title:translate['Info']||"Info" });
		});
	}
	//if logined to success page
	if (typeof success == "undefined" || !success) {
	...
	var msgapi = "./v2/srun_portal_message";
	if (portal.MsgApi == "old") {
		msgapi = "./v1/srun_portal_message";
	}
	//Message
	$.Message(msgapi, {}, function(data) {
	...
	$("#logout").click(function() {
		...
		$.Logout(host, params, function(data) {
			if (data.error == "ok") {
				isLogin = false;
				//Redirect Login Page
                location.href="./index_" + acid + ".html?srun_wait=1";
			} else {
				if (data.message == "NoResponseDataError") {
					showLog(username);
					return;
	...
	if (typeof success != "undefined" && success) {
		$.Info(host, {}, function(data) {
			if (data.error == "ok") {
				...
				$("#ip").html(data.ip);
				if (data.domain != "" && data.user_name.indexOf(data.domain) == -1) {
		...
		if ($("#visitor-qrcode").length > 0) {
			$("#visitor-qrcode").click(function() {
				...
				$.get(host + "/v1/srun_portal_proxy", params, function(response){
					if (response.code == 0) {
						layer.open({
							title: '',
							btn:[],
